...
class MyAdv extends Component {
  ...
  async componentWillMount() {
    ...
    I18nManager.forceRTL(true);
      ...
      if(this.props.auth){
        ...
        axios({
          url       : CONST.url + 'userAdvertisements',
          method    : 'POST',
          data      : { 
          ...
          let data = response.data.data.advertisements;
  
          if(data.length === 0){
            Toast.show({ text:'لا يوجد إعلانات', 
            duration    : 2000,
            type        : "danger",
            textStyle     : { 
                color       : "white",
  ...
  Delete_Adv(id,i){
    ...
    axios({
      url       : CONST.url + 'deleteUserAdvertisments',
      method    : 'POST',
      data      : { 
        advertisment_id     : id,
        user_id             : this.props.auth.data.user.id,
  ...
  static navigationOptions = () => ({
    header      : null,
    drawerLabel : ( <Text style={styles.textLabel}>إعلاناتي</Text> ) ,
    drawerIcon  : ( <Icon style={styles.icon} type="Foundation" name="megaphone" /> )
  });
      ...
      />
        ...
        <Header style={styles.header}>
          ...
          <Body style={styles.bodyText}>
            <Title style={styles.Title}>إعلاناتي</Title>
          </Body>
        </Header>
...

                {
                  this.state.blocks.map((block, i) => (
                        <View style={styles.blockItem}>
                            ...
                            <TouchableOpacity transparent style={styles.clear} onPress={()=>{this.Delete_Adv(block.id,i)}}>
...
export default connect(mapStateToProps, { userLogin ,profile })(MyAdv);